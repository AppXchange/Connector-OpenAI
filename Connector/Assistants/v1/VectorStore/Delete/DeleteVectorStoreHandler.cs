...
public class DeleteVectorStoreHandler : IActionHandler<DeleteVectorStoreAction>
{
    private readonly ILogger<DeleteVectorStoreHandler> _logger;

    public DeleteVectorStoreHandler(
        ILogger<DeleteVectorStoreHandler> logger)
    {
        _logger = logger;
    }
    ...
    {
        var input = JsonSerializer.Deserialize<DeleteVectorStoreActionInput>(actionInstance.InputJson);
        try
        {
            // Given the input for the action, make a call to your API/system
            var response = new ApiResponse<DeleteVectorStoreActionOutput>();
            // response = await _apiClient.PostVectorStoreDataObject(input, cancellationToken)
            ...
            // var resource = await _apiClient.GetVectorStoreDataObject(response.Data.id, cancellationToken);

            // var resource = new DeleteVectorStoreActionOutput
            // {
            //      TODO : map
            // };
        ...
        catch (HttpRequestException exception)
        {
            ...
            var errorSource = new List<string> { "DeleteVectorStoreHandler" };
