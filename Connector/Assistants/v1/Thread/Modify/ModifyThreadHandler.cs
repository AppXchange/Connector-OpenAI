...
public class ModifyThreadHandler : IActionHandler<ModifyThreadAction>
{
    private readonly ILogger<ModifyThreadHandler> _logger;

    public ModifyThreadHandler(
        ILogger<ModifyThreadHandler> logger)
    {
        _logger = logger;
    }
    ...
    {
        var input = JsonSerializer.Deserialize<ModifyThreadActionInput>(actionInstance.InputJson);
        try
        {
            // Given the input for the action, make a call to your API/system
            var response = new ApiResponse<ModifyThreadActionOutput>();
            // response = await _apiClient.PostThreadDataObject(input, cancellationToken)
            ...
            // var resource = await _apiClient.GetThreadDataObject(response.Data.id, cancellationToken);

            // var resource = new ModifyThreadActionOutput
            // {
            //      TODO : map
            // };
            ...
            {
                new CacheSyncCollection() { DataObjectType = typeof(ThreadDataObject), CacheChanges = operations.ToArray() }
            };
        ...
        catch (HttpRequestException exception)
        {
            ...
            var errorSource = new List<string> { "ModifyThreadHandler" };
