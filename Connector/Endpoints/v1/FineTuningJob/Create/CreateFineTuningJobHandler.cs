...
public class CreateFineTuningJobHandler : IActionHandler<CreateFineTuningJobAction>
{
    private readonly ILogger<CreateFineTuningJobHandler> _logger;

    public CreateFineTuningJobHandler(
        ILogger<CreateFineTuningJobHandler> logger)
    {
        _logger = logger;
    }
    ...
    {
        var input = JsonSerializer.Deserialize<CreateFineTuningJobActionInput>(actionInstance.InputJson);
        try
        {
            // Given the input for the action, make a call to your API/system
            var response = new ApiResponse<CreateFineTuningJobActionOutput>();
            // response = await _apiClient.PostFineTuningJobDataObject(input, cancellationToken)
            ...
            // var resource = await _apiClient.GetFineTuningJobDataObject(response.Data.id, cancellationToken);

            // var resource = new CreateFineTuningJobActionOutput
            // {
            //      TODO : map
            // };
        ...
        catch (HttpRequestException exception)
        {
            ...
            var errorSource = new List<string> { "CreateFineTuningJobHandler" };
