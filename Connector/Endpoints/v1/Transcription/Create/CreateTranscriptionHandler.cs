...
public class CreateTranscriptionHandler : IActionHandler<CreateTranscriptionAction>
{
    private readonly ILogger<CreateTranscriptionHandler> _logger;

    public CreateTranscriptionHandler(
        ILogger<CreateTranscriptionHandler> logger)
    {
        _logger = logger;
    }
    ...
    {
        var input = JsonSerializer.Deserialize<CreateTranscriptionActionInput>(actionInstance.InputJson);
        try
        {
            // Given the input for the action, make a call to your API/system
            var response = new ApiResponse<CreateTranscriptionActionOutput>();
            // response = await _apiClient.PostTranscriptionDataObject(input, cancellationToken)
            ...
            // var resource = await _apiClient.GetTranscriptionDataObject(response.Data.id, cancellationToken);

            // var resource = new CreateTranscriptionActionOutput
            // {
            //      TODO : map
            // };
            ...
            {
                new CacheSyncCollection() { DataObjectType = typeof(TranscriptionDataObject), CacheChanges = operations.ToArray() }
            };
        ...
        catch (HttpRequestException exception)
        {
            ...
            var errorSource = new List<string> { "CreateTranscriptionHandler" };
